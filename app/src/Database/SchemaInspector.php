<?php

declare(strict_types=1);

namespace Database;

use Core\Request;
use Core\Response;

/**
 * SchemaInspector — discovers database, table and column metadata.
 *
 * Uses ProfileManager to load stored credentials, opens a Connection,
 * then queries MySQL's information schema via standard SQL commands.
 */
class SchemaInspector
{
    private ProfileManager $profileManager;

    /** System databases to hide from the list returned by getDatabases(). */
    private const SYSTEM_DBS = ['information_schema', 'performance_schema', 'mysql', 'sys'];

    public function __construct()
    {
        $this->profileManager = new ProfileManager();
    }

    // -------------------------------------------------------------------------
    // getDatabases — returns a sorted list of non-system database names
    // -------------------------------------------------------------------------
    public function getDatabases(Request $request): void
    {
        $profileId = $request->get('profileId');
        if (!$profileId) {
            Response::error('profileId is required.', 400);
        }

        $pdo = $this->connectOrFail($profileId);

        try {
            $stmt      = $pdo->query('SHOW DATABASES');
            $databases = $stmt->fetchAll(\PDO::FETCH_COLUMN);

            // Filter out well-known system databases
            $databases = array_values(array_filter(
                $databases,
                fn($db) => !in_array(strtolower((string) $db), self::SYSTEM_DBS, true)
            ));

            sort($databases);
            Response::success($databases);
        } catch (\PDOException $e) {
            Response::error('Failed to fetch databases: ' . $e->getMessage());
        }
    }

    // -------------------------------------------------------------------------
    // getTables — returns a sorted list of table names for a profile
    //
    // Optional query param:
    //   database  — if supplied, list tables from that database instead of the
    //               profile's default database (SHOW TABLES FROM `db`)
    // -------------------------------------------------------------------------
    public function getTables(Request $request): void
    {
        $profileId = $request->get('profileId');
        $database  = (string) ($request->get('database') ?? '');

        if (!$profileId) {
            Response::error('profileId is required.', 400);
        }

        if ($database !== '' && !preg_match('/^\w+$/', $database)) {
            Response::error('Invalid database name.', 400);
        }

        $pdo = $this->connectOrFail($profileId);

        try {
            // Resolve database name if not supplied
            if ($database === '') {
                $database = (string) $pdo->query('SELECT DATABASE()')->fetchColumn();
            }

            if ($database === '') {
                Response::error('No database selected.', 400);
            }

            // Single query: get table names + approximate row counts from information_schema
            $stmt = $pdo->prepare(
                "SELECT TABLE_NAME, TABLE_ROWS
                 FROM information_schema.TABLES
                 WHERE TABLE_SCHEMA = ?
                   AND TABLE_TYPE = 'BASE TABLE'
                 ORDER BY TABLE_NAME"
            );
            $stmt->execute([$database]);

            $tables = [];
            while ($row = $stmt->fetch(\PDO::FETCH_ASSOC)) {
                $tables[] = [
                    'name' => $row['TABLE_NAME'],
                    'rows' => (int) ($row['TABLE_ROWS'] ?? 0),
                ];
            }

            Response::success($tables);
        } catch (\PDOException $e) {
            Response::error('Failed to fetch tables: ' . $e->getMessage());
        }
    }

    // -------------------------------------------------------------------------
    // getColumns — returns normalised column metadata for one table
    //
    // Each column: { name, type, shortType, nullable, key, default, extra }
    //   key values: "PRI" | "UNI" | "MUL" | ""
    //   extra:      "auto_increment" | "on update CURRENT_TIMESTAMP" | ""
    //
    // Optional query param:
    //   database  — if supplied, qualify the table reference as `db`.`table`
    // -------------------------------------------------------------------------
    public function getColumns(Request $request): void
    {
        $profileId = $request->get('profileId');
        $table     = $request->get('table');
        $database  = (string) ($request->get('database') ?? '');

        if (!$profileId) Response::error('profileId is required.', 400);
        if (!$table)     Response::error('table is required.', 400);

        // Whitelist: table names must be plain identifiers
        if (!preg_match('/^\w+$/', $table)) {
            Response::error('Invalid table name.', 400);
        }

        if ($database !== '' && !preg_match('/^\w+$/', $database)) {
            Response::error('Invalid database name.', 400);
        }

        $pdo = $this->connectOrFail($profileId);

        try {
            $tableRef = $database !== ''
                ? "`{$database}`.`{$table}`"
                : "`{$table}`";

            $stmt    = $pdo->query("DESCRIBE {$tableRef}");
            $columns = $stmt->fetchAll();

            $normalised = array_map(fn(array $col) => [
                'name'      => $col['Field'],
                'type'      => $col['Type'],
                'shortType' => $this->shortType($col['Type']),
                'nullable'  => $col['Null'] === 'YES',
                'key'       => $col['Key'],      // PRI | UNI | MUL | ""
                'default'   => $col['Default'],
                'extra'     => $col['Extra'],    // auto_increment | ""
            ], $columns);

            Response::success($normalised);
        } catch (\PDOException $e) {
            Response::error('Failed to fetch columns: ' . $e->getMessage());
        }
    }

    // -------------------------------------------------------------------------
    // getCreateStatement — returns the CREATE TABLE DDL for a single table
    //
    // Optional query param:
    //   database  — if supplied, qualify the table reference as `db`.`table`
    // -------------------------------------------------------------------------
    public function getCreateStatement(Request $request): void
    {
        $profileId = $request->get('profileId');
        $table     = $request->get('table');
        $database  = (string) ($request->get('database') ?? '');

        if (!$profileId) Response::error('profileId is required.', 400);
        if (!$table)     Response::error('table is required.', 400);

        if (!preg_match('/^\w+$/', $table)) {
            Response::error('Invalid table name.', 400);
        }

        if ($database !== '' && !preg_match('/^\w+$/', $database)) {
            Response::error('Invalid database name.', 400);
        }

        $pdo = $this->connectOrFail($profileId);

        try {
            $tableRef = $database !== ''
                ? "`{$database}`.`{$table}`"
                : "`{$table}`";

            $stmt = $pdo->query("SHOW CREATE TABLE {$tableRef}");
            $row  = $stmt->fetch(\PDO::FETCH_ASSOC);

            // MySQL returns the DDL in the second column ("Create Table")
            $ddl = $row['Create Table'] ?? $row[array_keys($row)[1]] ?? '';

            Response::success(['ddl' => $ddl]);
        } catch (\PDOException $e) {
            Response::error('Failed to fetch CREATE statement: ' . $e->getMessage());
        }
    }

    // -------------------------------------------------------------------------
    // getRowCounts — returns approximate row counts from information_schema
    //
    // Uses TABLE_ROWS from information_schema.TABLES (one fast metadata query).
    // For InnoDB this is an estimate; for MyISAM it is exact.
    //
    // Required body params:
    //   profileId  — connection profile
    //   tables     — JSON array of table names
    // Optional:
    //   database   — if omitted, uses the profile's default database
    // -------------------------------------------------------------------------
    public function getRowCounts(Request $request): void
    {
        $profileId = $request->get('profileId');
        $tables    = $request->get('tables') ?? [];
        $database  = (string) ($request->get('database') ?? '');

        if (!$profileId) Response::error('profileId is required.', 400);
        if (!is_array($tables) || empty($tables)) Response::error('tables array is required.', 400);

        foreach ($tables as $t) {
            if (!preg_match('/^\w+$/', (string) $t)) {
                Response::error('Invalid table name: ' . $t, 400);
            }
        }

        if ($database !== '' && !preg_match('/^\w+$/', $database)) {
            Response::error('Invalid database name.', 400);
        }

        $pdo = $this->connectOrFail($profileId);

        try {
            // Resolve database if not supplied
            if ($database === '') {
                $database = (string) $pdo->query('SELECT DATABASE()')->fetchColumn();
            }

            if ($database === '') {
                Response::error('No database selected.', 400);
            }

            $placeholders = implode(',', array_fill(0, count($tables), '?'));
            $stmt = $pdo->prepare(
                "SELECT TABLE_NAME, TABLE_ROWS
                 FROM information_schema.TABLES
                 WHERE TABLE_SCHEMA = ?
                   AND TABLE_NAME IN ({$placeholders})"
            );
            $stmt->execute([$database, ...$tables]);

            $counts = [];
            while ($row = $stmt->fetch(\PDO::FETCH_ASSOC)) {
                $counts[$row['TABLE_NAME']] = (int) ($row['TABLE_ROWS'] ?? 0);
            }

            Response::success($counts);
        } catch (\PDOException $e) {
            Response::error('Failed to fetch row counts: ' . $e->getMessage());
        }
    }

    // =========================================================================
    // Private helpers
    // =========================================================================

    /** Loads profile + opens PDO, or calls Response::error and exits. */
    private function connectOrFail(string $profileId): \PDO
    {
        $profile = $this->profileManager->getProfileById($profileId);
        if (!$profile) {
            Response::error('Profile not found.', 404);
        }

        try {
            return (new Connection($profile))->getPdo();
        } catch (\PDOException $e) {
            Response::error('Connection failed: ' . $e->getMessage());
        }
    }

    /** Strips length/precision from a MySQL type. "varchar(255)" → "varchar" */
    private function shortType(string $type): string
    {
        return strtolower(preg_replace('/\(.*\)/', '', $type));
    }
}
